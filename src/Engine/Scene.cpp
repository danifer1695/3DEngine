...
void Scene::Init()
{
	...
	ImportModel("sword", "./assets/models/sword/source/Sword.obj");
	...
	ImportTexture("sword_diff",		"./assets/models/sword/textures/albedo.png", false);
	ImportTexture("sword_spec",		"./assets/models/sword/textures/metallic.png", false);
	ImportTexture("sword_normal",	"./assets/models/sword/textures/normal.png", false);
	ImportTexture("sword_emiss",	"./assets/models/sword/textures/emissive.png", false);
	...
	ImportMaterialBlinn("sword", "./shaders/Blinn/blinn.vs", "./shaders/Blinn/blinn.fs", textures2D["sword_diff"], textures2D["sword_spec"], textures2D["sword_normal"], textures2D["sword_spec"], textures2D["sword_emiss"]);
	...
	items["sword"] = std::make_unique<Item>("Sword", models["sword"], materials["sword"]);
	...
	unsigned int emissive_map)
{
	//we check for duplicates
	if (materials.find(name) == materials.end())
		materials[name] = std::make_shared<MaterialBlinn>(name, vshaderPath, fshaderPath, diffuse_map, specular_map, normal_map, glossiness_map, emissive_map);
	else
		std::cout << "SCENE::IMPORT_MATERIAL::WARNING: Material with name " << name << " already exists!" << std::endl;
}
